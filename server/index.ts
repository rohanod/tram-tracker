...
async function nearestShortcutStops(ctx, req) {
  ...
  let stops = BUNDLED_TRANSIT_STOPS;
  const ownerId = ownerKeyFor(ctx);
  if (ownerId) {
    const stopIndex = await firstForOwners(ctx.db.transitStopIndexes, [ownerId]);
    if (stopIndex?.payload) {
      try {
        const uploadedStops = transitStopsFromMetadata(JSON.parse(stopIndex.payload));
        if (uploadedStops.length) stops = uploadedStops;
      } catch {
        // Keep the bundled stop index as the zero-downtime fallback.
      }
    }
  }
